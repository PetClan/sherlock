...
# debug ==================
@app.get("/api/v1/debug/app-blocks/{shop}")
async def debug_app_blocks(shop: str, db: AsyncSession = Depends(get_db)):
    """Temporary debug endpoint - shows raw theme data structure for app detection"""
    import json, httpx
    ...
    debug_info = {"shop": shop, "findings": []}
    
    async with httpx.AsyncClient() as client:
        # Get main theme
        themes_resp = await client.get(
            f"https://{shop}/admin/api/2024-01/themes.json",
        ...
        theme_id = main_theme["id"]
        debug_info["theme"] = {"id": theme_id, "name": main_theme.get("name")}
        ...
        asset = settings_resp.json().get("asset", {})
        settings_content = asset.get("value", "{}")
        settings_data = json.loads(settings_content)
        
        current = settings_data.get("current", {})
        debug_info["current_type"] = type(current).__name__
        
        if isinstance(current, str):
            debug_info["current_value"] = current
            presets = settings_data.get("presets", {})
            debug_info["presets_type"] = type(presets).__name__
            if isinstance(presets, dict):
                debug_info["preset_keys"] = list(presets.keys())
                resolved = presets.get(current, {})
                debug_info["resolved_type"] = type(resolved).__name__
                current = resolved if isinstance(resolved, dict) else {}
        
        if isinstance(current, dict):
            debug_info["current_top_keys"] = list(current.keys())
            
            # Check root blocks
            root_blocks = current.get("blocks", {})
            debug_info["root_blocks_type"] = type(root_blocks).__name__
            debug_info["root_blocks_count"] = len(root_blocks) if isinstance(root_blocks, dict) else 0
            
            # Check sections
            sections = current.get("sections", {})
            debug_info["sections_type"] = type(sections).__name__
            ...
            if isinstance(sections, dict):
                ...
                for sec_name, sec_data in sections.items():
                    if isinstance(sec_data, dict):
                        ...
                        section_summary[sec_name] = {"type": sec_type, "block_count": block_count}
                            ...
                            for bid, bdata in blocks.items():
                                if isinstance(bdata, dict):
                                    ...
                                    if "shopify://apps/" in btype:
                                        app_blocks_found.append({"section": sec_name, "block_type": btype})
                    else:
                        section_summary[sec_name] = {"type": type(sec_data).__name__, "block_count": 0}
                
                debug_info["sections_count"] = len(sections)
                debug_info["sections_summary"] = section_summary
                debug_info["app_blocks_in_settings"] = app_blocks_found
        
        # Check template JSONs
        ...
        template_jsons = [a["key"] for a in all_assets if a.get("key", "").startswith("templates/") and a.get("key", "").endswith(".json")]
        debug_info["template_json_files"] = template_jsons
        
        template_app_blocks = []
        for tmpl_key in template_jsons[:15]:
            try:
                tmpl_resp = await client.get(
                    f"https://{shop}/admin/api/2024-01/themes/{theme_id}/assets.json",
                    params={"asset[key]": tmpl_key},
                    headers={"X-Shopify-Access-Token": store.access_token, "Content-Type": "application/json"},
                    timeout=15.0
                )
                if tmpl_resp.status_code == 200:
                    tmpl_content = tmpl_resp.json().get("asset", {}).get("value", "{}")
                                    ...
                                    if isinstance(blocks, dict):
                                        for bid, bdata in blocks.items():
                                            if isinstance(bdata, dict):
                                                btype = bdata.get("type", "")
                                                if "shopify://apps/" in btype:
                                                    template_app_blocks.append({"template": tmpl_key, "section": sec_name, "block_type": btype})
            except:
                continue
        
        debug_info["app_blocks_in_templates"] = template_app_blocks
